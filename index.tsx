...
.btn-link {
    background: none; border: none; color: #8b949e; margin-top: 1rem;
    cursor: pointer; font-size: 0.85rem; text-decoration: underline; width: 100%;
}
...
function AdminPage() {
  ...
  const handleResetPassword = async () => {
      ...
      try {
          ...
          alert(`Um email de redefinição de senha foi enviado para ${email}. Verifique sua caixa de entrada (e spam).`);
      } catch (error: any) {
          console.error(error);
        ...
        <div style={{display:'flex', height:'100vh', alignItems:'center', justifyContent:'center', position: 'relative', zIndex: 10}}>
            <div className="form-card" style={{width: '400px'}}>
                ...
                <form onSubmit={handleLogin}>
                    ...
                    <button type="button" onClick={handleResetPassword} className="btn-link">Esqueci a senha</button>
                </form>
            </div>
        </div>
    )
